...
const fyers_1 = require("../brokers/fyers");
...
class AutoTradeEngine {
     ...
     */
    static async processSignal(signalId) {
        const signal = await mongooseSchema_1.Signal.findById(signalId);
        if (!signal)
        ...
        const activeUsers = await mongooseSchema_1.User.find({
    ...
    static async executeForUser(user, signal) {
        try {
            // 1. Check Risk Rules
            if (!this.checkRiskRules(user, signal)) {
                // Log rejection
                await mongooseSchema_1.Trade.create({
            ...
            const broker = new fyers_1.FyersBroker(user.fyersAppId, user.fyersAccessTokenEncrypted);
    ...
    static checkRiskRules(user, signal) {
        if (user.riskProfile.currentDailyLoss >= user.riskProfile.maxLossPerDay)
            return false;
        return true;
    }
}
