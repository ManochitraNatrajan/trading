...
class AuthService {
     ...
     */
    static async verifyOtpAndLogin(identifier, otp) {
        ...
        // 3. Generate JWT
        const token = jsonwebtoken_1.default.sign({ userId: user.id, role: 'user', subStatus: user.subscriptionStatus }, JWT_SECRET, { expiresIn: '7d' } // 7-day secure session
        );
        return { user, token };
    }
}
